...
class MinibatchDiscrimination(nn.Module):
    def __init__(self, in_feat_size, 
            out_channel, kernel_dims=16, mean=False):
        super(MinibatchDiscrimination, self).__init__()
        self.in_feat_size = in_feat_size
        self.input_length = in_feat_size[0] * in_feat_size[1] * in_feat_size[2]
        self.output_length = out_channel
        self.out_channel = out_channel
        self.kernel_dims = kernel_dims
        self.mean = mean
        self.T = nn.Parameter(torch.Tensor(
            self.input_length,
            self.output_length, kernel_dims))
        self.fuse_layer = nn.Conv2d(in_feat_size[0]+out_channel, 
                in_feat_size[0], kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, x):
        # x is NxA
        # T is AxBxC
        matrices = x.view(x.size(0), -1).mm(self.T.view(self.input_length, -1))
        matrices = matrices.view(-1, self.output_length, self.kernel_dims)
        ...
        x = torch.cat([x, o_b.view(x.size(0), 
            self.out_channel, 1, 1).expand(x.size(0), 
                self.out_channel, self.in_feat_size[1], self.in_feat_size[2])], 1)
...
class D_NLayersMulti(nn.Module):
    ...
    def forward(self, input, conditions=None):
...
class E_ResNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, ndf=64, n_blocks=4,
        ...
        if vaeLike:
            self.fc = nn.Sequential(*[nn.Linear(output_ndf * 2, output_nc)])
            self.fcVar = nn.Sequential(*[nn.Linear(output_ndf * 2, output_nc)])
        else:
            self.fc = nn.Sequential(*[nn.Linear(output_ndf * 2, output_nc)])
...
class G_Unet_add_all(nn.Module):
    ...
    def forward(self, x, extra_x, z=None):
        batch_size = x.size(0)

        if extra_x is not None: x = torch.cat((x, extra_x), dim=1);
        if z is None: 
            z = torch.randn(batch_size, self.nz)
        if cfg.CUDA: z = z.cuda()


        return self.model(x, z), z
