...
class Dataset(data.Dataset): 
    ...
    def __init__(self, data_dir, split='train', imsize=512, config=None, 
        ...
        if len(pre_set_list) == 0:
            ...
            self.files = ["%s.jpg" % x for x in pre_set_list]

    def __getitem__(self, index):
        file_name = self.files[index]

        combined_img = Image.open(os.path.join(self.split_dir, 
            file_name)).convert('RGB')
        image_array = np.asarray(combined_img)
        label_image = Image.fromarray(image_array[:, 256:, :])
        gt_image = Image.fromarray(image_array[:, :256, :])
        ...
        img_tensor = img_transform(gt_image)
        mask_tensor = img_transform(label_image)

        return img_tensor, mask_tensor, torch.FloatTensor([0,]), file_name
...
def get_transforms(params, method=Image.BILINEAR, normalize=False, split='train'):
