...
class Dataset(data.Dataset): 
    ...
    def __init__(self, data_dir, split='train', imsize=256, config=None):
        if config is None:
            self.config = cfg
        else:
            self.config = config
        ...
        self.night_dir = os.path.join(self.split_dir, "Night")
        self.day_dir = os.path.join(self.split_dir, "Day")
        all_night_files = os.listdir(self.night_dir)
        all_day_files = os.listdir(self.day_dir)
        
        self.night_files = all_night_files
        self.day_files = all_day_files

    def __getitem__(self, index):
        night_img_name = self.night_files[index]
        night_head = night_img_name.split('_')[0]
        
        day_candidates = [x for x in self.day_files \
                if x.startswith(night_head)]
        
        day_img_name = day_candidates[random.randint(0,
            len(day_candidates)-1)]

        night_img = Image.open(os.path.join(self.night_dir, 
            night_img_name)).convert('RGB')
        day_img = Image.open(os.path.join(self.day_dir, 
            day_img_name)).convert('RGB')
        ...
        night_tensor = img_transform(night_img)
        day_tensor = img_transform(day_img)

        # for placeholder
        place_holder = torch.FloatTensor([0,])

        return day_tensor, night_tensor, place_holder, night_img_name
