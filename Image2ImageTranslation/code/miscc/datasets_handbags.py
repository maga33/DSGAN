...
class Dataset(data.Dataset): 
    ...
    def __init__(self, data_dir, split='train', imsize=512, config=None, 
        ...
        if len(pre_set_list) == 0:
            ...
            self.filenames = ['%s_AB.jpg' % x for x in pre_set_list]
    ...
    def __getitem__(self, index):
        ...
        # lm_transform = get_transforms(trans_params, method=Image.BILINEAR, 
                # normalize=True, split=self.split)

        img_tensor = img_transform(pix_img)
        label_map_tensor = img_transform(label_img)
        # label_map_tensor = (1 - lm_transform(label_img)).sum(dim=0).clamp(max=1).\
                # unsqueeze(dim=0) # one channel image

        return img_tensor, label_map_tensor, torch.FloatTensor([0, ]), file_name
    # maintain the same output number
...
def get_transforms(params, method=Image.BILINEAR, normalize=False, split='train'):
    ...
    if normalize:
        trans_list.append(transforms.Normalize((0.5, 0.5, 0.5),
            (0.5, 0.5, 0.5)))
