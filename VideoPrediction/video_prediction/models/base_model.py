...
class VideoPredictionModel(BaseVideoPredictionModel):
    ...
    def tower_fn(self, inputs, targets=None):
        ...
        # MODIFIED
        #########################################
        gen_outputs_enc.pop('zs_for_loss', None)
        zs_for_loss = gen_outputs.pop('zs_for_loss', None)
        #########################################
        ...
        # MODIFIED
        ###################################
        outputs['zs_for_loss'] = zs_for_loss
        ###################################
    ...
    def compute_our_loss_fn(self, output_tuple, epsilon=1e-8):
        zs = output_tuple[2].pop('zs_for_loss', None)
        if zs is None:
            return 0.0
        gen_images = output_tuple[0]
        half_size = self.hparams.batch_size // (self.num_gpus * 2) # get correct segment size
        if self.hparams.zs_distance_type == 'l2':
            nominator = tf.reduce_mean(tf.squared_difference(gen_images[:half_size], gen_images[half_size:]), axis=[_i+1 for _i in range(gen_images.shape.ndims - 1)])
            denominator = tf.reduce_mean(tf.squared_difference(zs[:half_size, 0, :], zs[half_size:, 0, :]), axis=[_i+1 for _i in range(zs.shape.ndims - 2)])
        elif self.hparams.zs_distance_type == 'l1':
            nominator = tf.reduce_mean(tf.abs(gen_images[:half_size] - gen_images[half_size:]), axis=[_i+1 for _i in range(gen_images.shape.ndims - 1)])
            denominator = tf.reduce_mean(tf.abs(zs[:half_size, 0, :] - zs[half_size:, 0, :]), axis=[_i+1 for _i in range(zs.shape.ndims - 2)])
        denominator = tf.stop_gradient(denominator)
        our_term = -1.0 * tf.reduce_mean(nominator / (denominator + epsilon))
        return self.hparams.zs_lambda * (self.kl_weight if self.hparams.kl_weight else 1.0), our_term

    # compute our diversity measure
    def compute_our_diversity_fn(self, output_tuple):
        gen_samples = output_tuple[2].get('gen_images_samples', None)

        if gen_samples is None:
            return 0.0
        # should be (batch, generated_len, height, width, 3, num_samples)
        gen_samples = tf.transpose(gen_samples, [0, 5, 1, 2, 3, 4])

        tensor_shape = gen_samples.get_shape().as_list()

        # now (batch, num_samples, generated_len * height * width * 3)
        gen_samples = flatten(gen_samples, 2, -1)

        squared_form = tf.reduce_sum(gen_samples * gen_samples, 2)
        # turn r into column vector
        squared_form = tf.expand_dims(squared_form, -1)

        # do x^2 - 2xy + y^2
        squared_distance = squared_form - 2 * tf.matmul(gen_samples, tf.transpose(gen_samples, [0, 2, 1])) + tf.transpose(squared_form, [0, 2, 1])
        ...
        return tf.reduce_sum(tf.abs(squared_distance)) / (tensor_shape[0] * tensor_shape[1] * (tensor_shape[1] - 1)  * tensor_shape[2] * tensor_shape[3] * tensor_shape[4] * tensor_shape[5])
    #################################
    ...
    def build_graph(self, inputs, targets=None):
        ...
        if self.num_gpus <= 1:  # cpu or 1 gpu
            ...
            # MODIFIED
            ############################
            # compute our loss and add it to g_loss_post
            if self.mode != 'test':
                our_loss_mult, self.our_loss = self.compute_our_loss_fn(outputs_tuple)
                self.our_diversity = self.compute_our_diversity_fn(outputs_tuple)
                g_loss_post = g_loss_post + our_loss_mult * self.our_loss
            ############################
                ...
                with tf.variable_scope('', reuse=bool(i > 0)):
                    with tf.name_scope('tower_%d' % i):
                        with tf.device(device_setter):
                            ...
                            # compute our loss and add it to g_loss_post
                            if self.mode != 'test':
                                our_loss_mult, our_loss = self.compute_our_loss_fn(outputs_tuple)
                                tower_our_loss.append(our_loss)
                                g_loss_post = g_loss_post + our_loss_mult * our_loss

                                our_diversity = self.compute_our_diversity_fn(outputs_tuple)
                                tower_our_diversity.append(our_diversity)
                            ##############################
            ...
            with tf.device(consolidation_device):
                ...
                self.our_loss = reduce_tensors(tower_our_loss)
                self.our_diversity = reduce_tensors(tower_our_diversity)
        ...
        ###################################
        if self.mode != 'test':
            tf.summary.scalar("our_loss", self.our_loss)
            tf.summary.scalar("our_diversity", self.our_diversity)
        ###################################
    ...
    def generator_loss_fn(self, inputs, outputs, targets):
        ...
        # MODIFIED
        #############################
        # use kl_weight for our term only, not for the kl term in VAE
        #############################
