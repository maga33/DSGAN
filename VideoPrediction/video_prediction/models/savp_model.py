...
def generator_fn(inputs, outputs_enc=None, hparams=None):
    ...
    if hparams.nz:
        def sample_zs():
            if outputs_enc is None:
                ...
                # MODIFIED
                ############################
                # make z as same for single video
                ############################
                # original code
                if hparams.zs_lambda == -1.0:
                    zs = tf.random_normal([hparams.sequence_length - 1, batch_size, hparams.nz], 0, 1)
                else:
                    zs = tf.tile(tf.random_normal([1, batch_size, hparams.nz], 0, 1, seed=hparams.zs_seed_no), [hparams.sequence_length - 1, 1, 1])
                ############################
            else:
                ...
                if hparams.zs_lambda == -1.0:
                    ...
                    eps = tf.tile(tf.random_normal([1, batch_size, hparams.nz], 0, 1, seed=hparams.zs_seed_no), [hparams.sequence_length - 1, 1, 1])
    ...
    # MODIFIED
    #################################
    # make z as tracktable
    #################################
    outputs['zs_for_loss'] = inputs['zs']
    #################################
