...
class BaseVideoDataset(object):
    ...
    def make_batch(self, batch_size):
        ...
        dataset = dataset.map(self.parser, num_parallel_calls=max(batch_size // 2, 1))
        ...
        # MODIFIED
        #####################################
        # load half from dataset
        #####################################
        # original
        # dataset = dataset.batch(batch_size)
        #####################################
        dataset = dataset.batch(max(batch_size // 2, 1))
        #####################################
        ...
        # MODIFIED
        #####################################
        # concat batches, for recovery
        #####################################
        state_like_batches = {k:tf.concat([v, v], axis=0) for k, v in state_like_batches.items()}
        action_like_batches = {k:tf.concat([v, v], axis=0) for k, v in action_like_batches.items()}
        #####################################
